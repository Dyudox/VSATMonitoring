...
Partial Class createtask
    ...
    Dim com, comUpdate As SqlCommand
    ...
    Dim Notask, getIdStatusManager, namatask, tanggaltask, teknisi, email, project, tglmulai, tglselesai, permasalahan, catkoor, catman, getEmailKoord, getIDKoordinator, getIDMgr As String
    ...
    Private Sub createtask_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        'dstask.SelectCommand = "select a.rn,a.vid, tt.NamaTask, tt.NamaTeknisi, tt.NamaKoordinator, tt.CatatanManager, tt.IdStatusManager,tt.IdStatusKoordinator, tt.NoTask, tt.ID, tt.NomorPengaduan, tt.TanggalTask " &
        '                        " ,tt.LaporanPengaduan, tt.IdStatusTask, tt.TglStatusTask, " &
        '                        "tt.IdProject, tt.AsalPengaduan, tt.NamaPelapor, tt.TelpPelapor, " &
        '                        "tt.AlamatPengaduan, tt.DeskripsiPermasalahan, tt.IdCity, tt.IdProvinsi, tt.CatatanKoordinator  from ( " &
        '                        "select ROW_NUMBER() over (partition by vid order by vid asc) rn,vid,notask from trDetail_Task  " &
        '                        "where NoTask in (select notask from trTask where NomorPengaduan<>'') " &
        '                        ")as a " &
        '                        "left join trTask tt on tt.NoTask=a.NoTask " &
        '                        "where rn = 1 " &
        '                        "order by NoTask desc "
    End Sub
    ...
    Protected Sub grid_manager_RowUpdating(sender As Object, e As DevExpress.Web.Data.ASPxDataUpdatingEventArgs) Handles grid_manager.RowUpdating
        ...
        Try
            ...
            If IsDBNull(tbldata.Rows(0).Item("IdStatusManager")) = False Then
                If tbldata.Rows(0).Item("IdStatusManager") = "Valid" AndAlso e.NewValues("IdStatusManager") <> "Valid" Then
                    e.Cancel = True
                    grid_manager.CancelEdit()
                    'grid_manager.JSProperties("cpMessageError") = "Data tidak dapat diubah karena status sudah Valid."
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "uploadValid", "alert('Data tidak dapat diubah karena status sudah Valid.');", True)
                    grid_manager.DataBind()   ' <-- tambahkan ini
                    Exit Sub
                ElseIf tbldata.Rows(0).Item("IdStatusManager") = "Valid" AndAlso e.OldValues("IdStatusManager") <> "Valid" Then
                    e.Cancel = True
                    grid_manager.CancelEdit()
                    'grid_manager.JSProperties("cpMessageError") = "Data tidak dapat diubah karena status sudah Valid."
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "uploadValid", "alert('Data tidak dapat diubah karena status sudah Valid.');", True)
            ...
            Dim getID As String = e.NewValues("IdProject").ToString()
            Dim getTanggalTask As String = e.NewValues("TanggalTask").ToString()
            Dim getNamaTask As String = e.NewValues("NamaTask").ToString()
            Dim getNamaTeknisi As String = e.NewValues("NamaTeknisi").ToString()
            Dim getTglMulai As String = e.NewValues("TglMulai").ToString()
            Dim getTglSelesai As String = e.NewValues("TglSelesai").ToString()
            Dim getDeskripsiPermasalahan As String = e.NewValues("DeskripsiPermasalahan").ToString()
            Dim getIdStatusTask As String = e.NewValues("IdStatusTask").ToString()
            ...
            strsql = "UPDATE trTask SET IdProject=@IdProject, TanggalTask=@TanggalTask, NamaTask=@NamaTask, " &
                    "NamaTeknisi=@NamaTeknisi, TglMulai=@TglMulai, TglSelesai=@TglSelesai, " &
                    "DeskripsiPermasalahan=@DeskripsiPermasalahan, IdStatusTask=@IdStatusTask, " &
                    "IdUserManager='" & Session("username") & "', CatatanKoordinator='" & clsg.ReplacePetik(e.NewValues("CatatanKoordinator")) & "', " &
                    "CatatanManager='" & clsg.ReplacePetik(e.NewValues("CatatanManager")) & "', NamaKoordinator='" & Trim(e.NewValues("NamaKoordinator")) & "', " &
                    "IdStatusManager=@IdStatusManager, TglStatusManager=GETDATE(), " &
                    "AlamatPengaduan='" & clsg.ReplacePetik(e.NewValues("AlamatPengaduan")) & "' " &
                    "WHERE NoTask=@NoTask"

            clsg.writedata(Session("username"), "Update", "Task Manager", strsql, "")
            dsmanager.UpdateCommand = strsql

            Dim getdataemail As String = "SELECT trTask.NoTask, trTask.IdStatusManager, trTask.IdKoordinator, trTask.IdUserManager, msEmployee.Email, msEmployee.NIK, msEmployee.EmployeeType, trTask.IdProject, trTask.NamaTask, trTask.TanggalTask, trTask.NamaTeknisi,msEmployee.IdStatusPegawai, msEmployee.Email, trProject.ProjectName, " &
            ...
            getEmailKoord = dr("Email").ToString
            getIDKoordinator = dr("IdKoordinator").ToString
            getIDMgr = dr("IdUserManager").ToString
            getIdStatusManager = dr("IdStatusManager").ToString
    ...
    '            Using daEmail As New SqlDataAdapter(cmdEmail)
    ...
    '            Body.AppendLine("Project     : " & row("ProjectName").ToString())
    '            Body.AppendLine("Nama Task   : " & row("NamaTask").ToString())
    '            Body.AppendLine("Teknisi     : " & row("NamaTeknisi").ToString())
    '            Body.AppendLine("Tanggal     : " & CDate(row("TanggalTask")).ToString("dd-MM-yyyy"))
    '            Body.AppendLine()
    '            Body.AppendLine("Catatan Koordinator : " & row("CatatanKoordinator").ToString())
    ...
    ByVal bodyHtml As String
        ...
        Try
            ...
            Using con As New SqlConnection(connStr)
                ...
                Dim subjectEmail As String = String.Join(";", subject)

                Dim sql As String = "INSERT INTO ICC_EMAIL_OUT " &
                "(EMAIL_ID, DIRECTION, EFROM, ETO, ECC, ESUBJECT, EBODY_HTML, ROUTED, HANDLED, Email_Date, Flag, UserID, JENIS_EMAIL) " &
                "VALUES " &
                "(@EMAIL_ID, @DIRECTION, @EFROM, @ETO, @ECC, @ESUBJECT, @EBODY_HTML, @ROUTED, @HANDLED, @Email_Date, @Flag, @UserID, @JENIS_EMAIL)"
                ...
                Using cmd As New SqlCommand(sql, con)
                    ...
                    cmd.Parameters.AddWithValue("@JENIS_EMAIL", "STATUS VALID MGR")
    ...
    Protected Sub Grid_Validasi_RowUpdating(ByVal sender As Object, ByVal e As DevExpress.Web.Data.ASPxDataUpdatingEventArgs)
        Try
            If e.NewValues("IdStatusKoordinator").ToString = "Valid" Then
                dsstatusvalidasi.UpdateCommand = "update trtask set IdStatusKoordinator = @IdStatusKoordinator, estimasiBiaya=@estimasiBiaya,TglStatusKoordinator=getdate(), " &
                    "IdStatusManager='Valid', TglStatusManager=getdate(), CatatanManager='Laksanakan Segera!', IdUserManager='233019505' where ID=@ID"
            Else
                dsstatusvalidasi.UpdateCommand = "update trtask set IdStatusKoordinator = @IdStatusKoordinator, estimasiBiaya=@estimasiBiaya,TglStatusKoordinator=getdate() where ID=@ID"
            End If
            dsstatusvalidasi.DataBind()
        Catch ex As Exception
